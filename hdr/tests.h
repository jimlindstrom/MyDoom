...
#define TEST_ASSERT_EQUAL(x,y) if((x)!=(y)) { printf("TEST FAILED in %s (%s:%d), expected %.1f == %.1f\n", __PRETTY_FUNCTION__, __FILE__, __LINE__, (float)x, (float)y); tests_inc_failures(); } else { tests_inc_successes(); }

#define TEST_ASSERT_LESS_THAN(x,y) if((x)>=(y)) { printf("TEST FAILED in %s (%s:%d), expected %.1f < %.1f\n", __PRETTY_FUNCTION__, __FILE__, __LINE__, (float)x, (float)y); tests_inc_failures(); } else { tests_inc_successes(); }

#define TEST_ASSERT_GREATER_THAN(x,y) if((x)<=(y)) { printf("TEST FAILED in %s (%s:%d), expected %.1f > %.1f\n", __PRETTY_FUNCTION__, __FILE__, __LINE__, (float)x, (float)y); tests_inc_failures(); } else { tests_inc_successes(); }
