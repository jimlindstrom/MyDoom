...
void sidedef::render(float ldx_l, float ldx_r, int ld_h, int x_l, int x_r, float yt_l, float yb_l, float yt_r, float yb_r) const
{
  if(mid_texture)
  {
    printf("        SD rendering mid: \"%s\"\n", mid_texture_name);
    mid_texture->render(ldx_l, ldx_r, ld_h, x_l, x_r, yt_l, yb_l, yt_r, yb_r);
  }
  else if(upper_texture)
  {
    printf("        SD rendering upper: \"%s\"\n", upper_texture_name);
    upper_texture->render(ldx_l, ldx_r, ld_h, x_l, x_r, yt_l, yb_l, yt_r, yb_r);
  }
  else if(lower_texture)
  {
    printf("        SD rendering lower: \"%s\"\n", lower_texture_name);
    lower_texture->render(ldx_l, ldx_r, ld_h, x_l, x_r, yt_l, yb_l, yt_r, yb_r);
  }
}
