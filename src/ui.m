...
CGColorSpaceRef indexedColorsRef = NULL;
...
void ui_set_palette(unsigned char *new_colors)
{
  if (colorSpaceRef != NULL) { CGColorSpaceRelease(colorSpaceRef); }
  if (indexedColorsRef != NULL) { CGColorSpaceRelease(indexedColorsRef); }
  memcpy(colors, new_colors, 256*3);
  colorSpaceRef = CGColorSpaceCreateDeviceRGB();
  indexedColorsRef = CGColorSpaceCreateIndexed(colorSpaceRef, 255, colors);
}

void ui_draw_image(unsigned char *image_data, int width, int height)
{
  CGDataProviderRef provider = CGDataProviderCreateWithData(NULL,image_data,width*height,NULL);
