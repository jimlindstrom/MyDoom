...
bool sprite::set_from_lump_data(uint8_t const *data)
{
  ...
  for(y=0; y<height; y++)
  {
    ...
    while(rowstart != 255)
    {
      ...
      for(j=0; j<pixel_count; j++)
      {
        ...
        if(x<0 || x>=width)
        {
          printf("ERROR: sprite tried to write to (%d,%d) > (%d,%d)\n", x,y,width,height);
          return false;
        }
...
uint8_t const sprite::get_pixel(int x, int y)
{
  // FIXME: assert x,y are sane
  return pixel_columns[y][x];
}

void sprite::print_html_file(char const *filename, palette const *pal)
{
  FILE *f;
